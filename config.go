...
func loadConfig() *ConfigST {
	...
	db, err := sql.Open("mysql", "root:waheed786@tcp(172.17.0.1:3306)/cam")
	if err != nil {
		log.Fatalln("Error connecting to database:", err)
	}
	defer db.Close()

	// Query to get server http_port (assuming it's part of the configuration)
	// err = db.QueryRow("SELECT http_port FROM server_config LIMIT 1").Scan(&tmp.Server.HTTPPort)
	// if err != nil {
	// 	log.Fatalln("Error getting server HTTP port:", err)
	// }

	tmp.Server.HTTPPort = ":8083" // Static port value

	// Query to get active stream data
	rows, err := db.Query("SELECT name, url FROM active_streams WHERE active = 1")
	if err != nil {
	...
	// Populate Streams map with data from active_streams table
	for rows.Next() {
		var streamName, url string
		err := rows.Scan(&streamName, &url)
		if err != nil {
			log.Fatalln("Error scanning row:", err)
		}

		// Add the stream to the Streams map
		tmp.Streams[streamName] = StreamST{
			URL:              url,
			Status:           true,  // You can set default status here
			OnDemand:         false, // Set default on_demand value if needed
	...
	if err = rows.Err(); err != nil {
		log.Fatalln("Error after iterating rows:", err)
	}
